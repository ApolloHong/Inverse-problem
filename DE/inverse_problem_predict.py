...
def load_power18480(name='18480'):
    ...
    parameters = pd.read_csv('../data/inpower18480.out', header=None, delimiter=' ', dtype=float).iloc[:, [1, 2, 3, 4]]
    parameters.rename(columns={1: 'st', 2: 'bu', 3: 'pw', 4: 'tin'}, inplace=True)
    sensors = pd.read_csv('../data/SensorofQuarter.txt', header=None, delimiter=' ', dtype=float)
    observations = field.dot(sensors.T)
    return parameters, field, observations, sensors


def load_power18480_pod(dim=40):
    qbasis = pd.read_csv('../data/powerIAEA18480basis.txt',  delimiter=' ', header=None, dtype=float)  # shape:(n, M), n=150, M=52*28=1456
    basis = qbasis.iloc[:dim, :]  # DIM, M
    coefficient = pd.read_csv('../data/powerIAEA18480coef.txt', header=None, delimiter=' ', dtype=float)
    return basis, coefficient


def field_error(field, field_true):
    '''
...
def reconstruct_error_pod(coefficient, qbasis, original_field):
    ...
    return field_error(field, original_field)


def restruct_filed_by_inputs(inputs, model, basis):
    '''
...
def restruct_observation_error_from_input(inputs, model, basis, observation_true, sensors):
    '''
    ...
    :return:
    '''
    field = restruct_filed_by_inputs(inputs, model, basis)
    observation = np.dot(field, sensors.T)
    return field_error(observation, observation_true)


def restruct_compress_observation_error_from_input(inputs, model, basis, observation_true, sensors, compress):
    '''
    ...
    '''
    field = restruct_filed_by_inputs(inputs, model, basis)
    return field_error(compress.transform(field), observation_true)


def restruct_field_error_from_input(inputs, model, basis, filed_true):
    '''
    Reconstructs a field from `inputs` using `model` and `basis`,
    ...
    '''
    field = restruct_filed_by_inputs(inputs, model, basis)
    return field_error(field, filed_true)
...
def generate_anchors(bin={"bu":10, "st": 20, "pw": 10, "tin": 10}):
    ...
    The `bin` dictionary specifies the number of bins to use for each parameter.
    :param bin:
    :return:
    '''
    anchors = {}
...
def predict_parameter_from_observation(argv):
    ...
    pd.to_pickle(results, 'result/' + str(nc_start) + 'inverse_problem' + now + '.pkl')
    joblib.dump(models, 'result/' + str(nc_start) + 'inverse_problem_models' + now + ".pkl")


def select_best_models(s, params, fname='result/inverse_problem_best_models.pkl'):
    '''
...
def de_parameter_from_observation_5cases(runs, prefix, record, initial=False):
    ...
    for i in mus_indx:
        ...
        # observation error
        fobj = lambda p: restruct_observation_error_from_input(p, knn, basis, obs, sensors)
        # filed L2 error
        f_field_L2_error = lambda p: restruct_field_error_from_input(p, knn, basis, field_i)
        f_field_linf_error = lambda p: np.abs(restruct_filed_by_inputs(p, knn, basis) - field_i).max() / field_i.max()
    ...
    now = datetime.datetime.now().strftime("%m%d")
    joblib.dump(exp_results, 'result/'+prefix+'_'+now+'.pkl')
    return de_results
...
def initilize_pop(popsize, bounds, x = None):
    '''
...
def de_predict_parameter_from_observation(sigmas, num, prefix, noise, record):
    ...
    for s in sigmas:
        ...
        for i in mus_indx:
            ...
            x0 = np.concatenate((y, [295]))
            bounds = get_bounds(y, [20, 100, 15], limits=limits) + [(290, 300)]

            # generate initial population
            pop0 = initilize_pop(60, bounds, x=x0)
